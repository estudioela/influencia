...
| **(Achado de SSH real do responsável do projeto, 2026-07-23, `VALIDACAO_AMBIENTE_REAL.md`)** No host, o binário genérico `php` **não existe no PATH** — só `php83`. `public_html` confirmado vazio (nenhum deploy anterior); Composer e Git também confirmados por essa via (Composer ausente, Git presente) | **Pendente de correção:** `scripts/deploy-locaweb.sh` ainda chama `php artisan migrate/config:cache/route:cache/view:cache` com o binário genérico (linhas 35-38) — falharia no primeiro deploy real. Correção (`php` → `php83`) já desenhada, não aplicada (fora do escopo de documentação; ver `TASK_ROUTER.md`) |
