...
> Data: 2026-07-07. Baseado em `docs/AUDITORIA_TECNICA_2026-07-07.md` e leitura direta do código (`mae/*.js`, `mae/*.gs`, `mae/Index.html`).
>
> **STATUS (atualizado em 2026-07-08): a estratégia abaixo FOI IMPLEMENTADA.** A suíte existe em `test/` — 13 arquivos, **163 testes**, todos passando (`npm test`), rodando em CI via `.github/workflows/tests.yml`. O texto original deste documento (“nenhum teste foi implementado ainda”, “para aprovação antes da execução”) descrevia o estado de 2026-07-07 e ficou obsoleto no dia seguinte. Leia daqui em diante como **especificação da suíte existente**, não como proposta.
...
- Testes de **integração/E2E reais** só podem rodar de duas formas: (a) dentro do próprio Apps Script, via uma função "orquestradora" chamada manualmente ou por trigger (é o que `QaShadow.js` já faz parcialmente), ou (b) via HTTP contra uma deployment real (`doGet`; o `doPost` foi removido em 2026-07-07), de fora do Apps Script.
...
|---|---|---|
| `calcularDataAprovacao(dataInput)` | `Código.js` ~L316 | Sim |
| `getHeaderMap(sh)` | `Código.js` ~L959 | Sim (mock de `sh` simples) |
| `montarLinha(h, campos)` | `Código.js` ~L970 | Sim |
| `parseMesAno(textoBruto)` | `Código.js` ~L982 | Sim |
| `textToNumber(t)` | `Código.js` ~L992 | Sim |
| `formatarTitleCase(t)` | `Código.js` ~L1006 | Sim |
| `normalizarStatusAtivacao(statusBruto)` | `WebApp.js` ~L747 | Sim |
| `normalizarStatusPagamento(statusBruto)` | `WebApp.js` ~L765 | Sim |
| `extrairValorNumerico(valorStr)` | `WebApp.js` ~L782 | Sim |
| `nomeFormatoPasta(formato)` | `WebApp.js` ~L798 | Sim |
| `formatarData(data)` | `WebApp.js` ~L774 | Quase — usa `Utilities.formatDate` (1 mock simples) |
...
| **E2E (HTTP)** | `doGet` real (o `doPost` desta linha foi removido em 2026-07-07), jornada completa via chamadas HTTP | Node.js (`fetch`/`axios`) + Jest, contra deployment de QA | Local/CI, apontando para ambiente de QA | Antes de deploy em produção |
...
| **Node `fetch`/`axios` + Jest** | E2E via HTTP direto (`doGet`) sem navegador | Mais rápido que Playwright para validar contrato JSON puro; complementa (não substitui) o E2E de navegador |
