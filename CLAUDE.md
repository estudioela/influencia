...
- **Envio de material**: `mae/Index.html` (`arquivoSelecionado`/`iniciarEnvio`/`enviarArquivoResumable()` ~L1334) → `mae/WebApp.js:iniciarEnvioResumable()`/`finalizarEnvioResumable()` (~L822/862) → Drive (pasta por influenciadora via `BASE DE DADOS.PASTA_DRIVE_LINK`, `obterOuCriarPastaInfluenciadoraPorLinha()` — revertido de `PropertiesService` em 2026-07-06, pedido do usuário) + aba `ATIVAÇÕES`.
