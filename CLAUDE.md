...
- **Incidente 2026-07-08 — o commit `897e2dc` não é o que a mensagem diz.** O squash do PR #16 tem título `docs: realinha a V2...` e corpo afirmando "Nenhum código alterado", mas contém **8.010 inserções em 33 arquivos**: `mae/Código.js`, `mae/WebApp.js`, `mae/Index.html`, `mae/SchemaExporter.js`, `package.json` e a suíte de testes inteira. Causa: a branch do PR foi criada a partir do `main` **local**, que estava 8 commits à frente do `main` remoto — o trabalho de código da V1 fora commitado localmente e deployado via `clasp`, mas nunca empurrado ao GitHub. O squash colapsou os 8 commits + a documentação em um só. **Nada foi perdido e produção não foi afetada**: `origin/main` ficou com o conteúdo íntegro e os deploys `@34`–`@37` batem com o repo. Histórico original preservado na tag **`pre-squash-main-20260708`** (commits `111dea8..d323ae2`, com as mensagens descritivas de cada um). Não corrigível por reescrita — `main` é protegido (sem force-push, `enforce_admins`). **Lição operacional**: antes de criar branch, conferir `git status --short --branch` contra a origin — `ahead N` significa que o PR vai carregar N commits que você não pretende incluir.
